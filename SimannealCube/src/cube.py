# from https://github.com/rvaccarim/genetic_rubik
# author: rvaccarim
...
class Cube:
    ...
    def __init__(self):
        ...
        self.faces = {
            FRONT: np.full((3, 3), Color.F),
            LEFT: np.full((3, 3), Color.L),
            RIGHT: np.full((3, 3), Color.R),
            UP: np.full((3, 3), Color.U),
            DOWN: np.full((3, 3), Color.D),
            BACK: np.full((3, 3), Color.B),
        }
    ...
    def execute(self, moves: 'List'):
        ...
        # self.__calculate_fitness_euclidean()
        # self.__calculate_fitness_hausdorff()
        # self.__calculate_fitness_hausdorff_02()
    ...
    def calculate_fitness_euclidean(self):
        distance = 0
        ...
        for k, face in self.faces.items():
            ...
            for i in range(0, 3):
                for j in range(0, 3):
                    if face[i, j] != center:
                        distance += (face[i, j]-center) ** 2

        self.fitness = np.sqrt(distance)

    def calculate_fitness_hausdorff(self):
        face_min_distance = {}
        for k in self.faces.keys():
            face_min_distance[k] = 99999
        ...
        for k, face in self.faces.items():
            ...
            if(face_min_distance[k] > distance):
                face_min_distance[k] = np.sqrt(distance)

        self.fitness = max(face_min_distance.values())

    def calculate_fitness_hausdorff_02(self):
        face_min_distance = {}
        for k in self.faces.keys():
            face_min_distance[k] = 99
    ...
    def D_prime(self):
        self.faces[DOWN] = np.rot90(
            self.faces[DOWN], axes=COUNTERCLOCKWISE)
    ...
    def U_prime(self):
        self.faces[UP] = np.rot90(self.faces[UP], axes=COUNTERCLOCKWISE)
    ...
    def __swap_x(self, t1, t2, t3, t4):
        ...
        self.__copy_stickers(
            self.faces[t4[0]][t4[1]], self.faces[t3[0]][t3[1]])
        self.__copy_stickers(
            self.faces[t3[0]][t3[1]], self.faces[t2[0]][t2[1]])
        self.__copy_stickers(
            self.faces[t2[0]][t2[1]], self.faces[t1[0]][t1[1]])
    ...
    def L(self):
        ...
        self.__swap_y((DOWN, 0, True), (BACK, 2, True),
                      (UP, 0, False), (FRONT, 0, False))

    def L_prime(self):
        self.faces[LEFT] = np.rot90(self.faces[LEFT], axes=COUNTERCLOCKWISE)
        self.__swap_y((DOWN, 0, False), (FRONT, 0, False),
                      (UP, 0, True), (BACK, 2, True))
    ...
    def M(self):
        self.__swap_y((DOWN, 1, True), (BACK, 1, True),
                      (UP, 1, False), (FRONT, 1, False))

    def M_prime(self):
        self.__swap_y((DOWN, 1, False), (FRONT, 1, False),
                      (UP, 1, True), (BACK, 1, True))
    ...
    def R(self):
        ...
        self.__swap_y((DOWN, 2, False), (FRONT, 2, False),
                      (UP, 2, True), (BACK, 0, True))

    def R_prime(self):
        self.faces[RIGHT] = np.rot90(self.faces[RIGHT], axes=COUNTERCLOCKWISE)
        self.__swap_y((DOWN, 2, True), (BACK, 0, True),
                      (UP, 2, False), (FRONT, 2, False))
    ...
    def __swap_y(self, t1, t2, t3, t4):
        ...
        if t3[2]:
            self.__copy_stickers(self.faces[t4[0]][:, t4[1]], np.flip(
                self.faces[t3[0]][:, t3[1]]))
        else:
            self.__copy_stickers(
                self.faces[t4[0]][:, t4[1]], self.faces[t3[0]][:, t3[1]])

        if t2[2]:
            self.__copy_stickers(self.faces[t3[0]][:, t3[1]], np.flip(
                self.faces[t2[0]][:, t2[1]]))
        else:
            self.__copy_stickers(
                self.faces[t3[0]][:, t3[1]], self.faces[t2[0]][:, t2[1]])

        if t1[2]:
            self.__copy_stickers(self.faces[t2[0]][:, t2[1]], np.flip(
                self.faces[t1[0]][:, t1[1]]))
        else:
            self.__copy_stickers(
                self.faces[t2[0]][:, t2[1]], self.faces[t1[0]][:, t1[1]])
    ...
    def B(self):
        ...
        self.__swap_z((DOWN, 2, True), (RIGHT, 2, False),
                      (UP, 0, True), (LEFT, 0, False))

    def B_prime(self):
        self.faces[BACK] = np.rot90(self.faces[BACK], axes=COUNTERCLOCKWISE)
        self.__swap_z((DOWN, 2, False), (LEFT, 0, True),
                      (UP, 0, False), (RIGHT, 2, True))
    ...
    def F(self):
        ...
        self.__swap_z((DOWN, 0, False), (LEFT, 2, True),
                      (UP, 2, False), (RIGHT, 0, True))

    def F_prime(self):
        self.faces[FRONT] = np.rot90(self.faces[FRONT], axes=COUNTERCLOCKWISE)
        self.__swap_z((DOWN, 0, True), (RIGHT, 0, False),
                      (UP, 2, True), (LEFT, 2, False))
    ...
    def S(self):
        self.__swap_z((DOWN, 1, False), (LEFT, 1, True),
                      (UP, 1, False), (RIGHT, 1, True))

    def S_prime(self):
        self.__swap_z((DOWN, 1, True), (RIGHT, 1, False),
                      (UP, 1, True), (LEFT, 1, False))
    ...
    def __swap_z(self, t1, t2, t3, t4):
        ...
        if t3[2]:
            self.__copy_stickers(
                self.faces[t4[0]][:, t4[1]], np.flip(self.faces[t3[0]][t3[1]]))
        else:
            self.__copy_stickers(
                self.faces[t4[0]][:, t4[1]], self.faces[t3[0]][t3[1]])
        ...
        if t1[2]:
            self.__copy_stickers(
                self.faces[t2[0]][:, t2[1]], np.flip(self.faces[t1[0]][t1[1]]))
        else:
            self.__copy_stickers(
                self.faces[t2[0]][:, t2[1]], self.faces[t1[0]][t1[1]])
    ...
    def get_algorithm(self):
        ...
        flat_list = [item for sublist in self.move_history[1:]
                     for item in sublist]
        return flat_list
    ...
    def __str__(self):
        state = [f"Scramble: {self.get_scramble_str()}",
                 f"Algorithm: {self.get_algorithm_str()}"]

        for k, v in sorted(self.faces.items()):
            state.append(
                f"{'{:<8}'.format(k + ':')}  {v[0, 0]} {v[0, 1]} {v[0, 2]}")
            state.append(
                f"{'{:<8}'.format('')}  {v[1, 0]} {v[1, 1]} {v[1, 2]}"),
            state.append(
                f"{'{:<8}'.format('')}  {v[2, 0]} {v[2, 1]} {v[2, 2]}")
